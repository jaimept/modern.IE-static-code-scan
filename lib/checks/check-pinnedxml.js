...
var check = function (website) {
    ...
    if (xml.length > 0) {
        ...
        // PUESTO A LA BRAVA, el path es correcto (?), pero no carga el archivo XML o no hace el parsing bien (?)
        url = url + 'IEconfig.xml';
        // ESTO DEBERÍA SER ALGO PARECIDO, pero no funciona
        // url = url + xml[0].content;
        console.log(url);
        request(url, function (error, response, body) {
            if (!error && response.statusCode === 200) {
                if (body.indexOf('square70x70') !== -1) {
                    ...
                    console.log('ERROR square70');
                    test.passed = false;
                    test.data.square70 = false;
                }
            }
            else {
                console.log('ERROR square70, square150, wide310 and square310');
                test.passed = false;
                test.data.square70 = false;
